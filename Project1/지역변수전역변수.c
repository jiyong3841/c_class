...
int main(void)
{
	...
	/*
	지역변수 
	1. 중괄호 (함수 스코프)내에 선언된 모든 변수는 해당 함수에서만 사용 가능하다.
	2. 함수를 실행할 때 사용하는 재료인 매개변수도 결국 지역변수이다.
	3. 지역변수는 자신이 선언된 지역 외 다른 지역(함수)에서는 사용이 불가능한다.
	...
	4. 프로그램이 시작됨과 동시에 메모리 공간이 할당되며, 프로그램이 종료료될 때까지 존재한다.

	스테틱변수 
	1.  static 이라는 키워드를 사용해서 선언한다.
	2. 지역변수, 전역변수 모두 선언 가능
	3. 지역변수와 함께사용하면 함수를 호출하고 기능이 끝나도 스테틱으로 선언한 변수는 메로리가 사라지지 않음
	4. 프로그램이 종료될 때까지 메모리 공간이 유지
	*/
	return 0;
}